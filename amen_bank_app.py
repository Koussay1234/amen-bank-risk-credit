...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        <div style="text-align:center;padding:1.2rem 0 .5rem">
          <img src="{LOGO_SRC}" style="height:64px;width:64px;border-radius:50%;border:3px solid {OR};margin-bottom:8px"/>
          <div style="font-family:'Playfair Display',serif;font-size:1rem;color:{OR};font-weight:700">{u['name']}</div>
          <div style="font-size:.72rem;color:rgba(255,255,255,.6);margin-top:2px">{u['role']}</div>
          <div style="background:rgba(245,166,35,.15);border-radius:20px;padding:2px 12px;display:inline-block;margin-top:6px;font-size:.67rem;color:{OR};border:1px solid {OR}55">
            🟢 En ligne · {st.session_state.username}</div>
        </div>
        <hr style="border-color:{OR}44;margin:.5rem 0 .9rem">""", unsafe_allow_html=True)
        ...
        st.markdown(f"""
        <div style="background:rgba(245,166,35,.1);border-radius:10px;padding:.8rem;border:1px solid {OR}44;margin-bottom:.9rem">
          <div style="font-size:.65rem;color:{OR};font-weight:700;text-transform:uppercase;letter-spacing:1px">📁 Dataset</div>
          <div style="font-size:.86rem;margin-top:4px">German Credit — ML Ready</div>
          <div style="font-size:.68rem;color:rgba(255,255,255,.5)">1 000 clients · Labels réels</div>
        </div>
        <div style="background:rgba(26,79,160,.15);border-radius:10px;padding:.7rem;border:1px solid {BLEU}44;margin-bottom:.9rem">
          <div style="font-size:.65rem;color:#93C5FD;font-weight:700;text-transform:uppercase;letter-spacing:1px">🚀 Version</div>
          <div style="font-size:.82rem;margin-top:2px;color:{BLANC}">v4.0 Professional</div>
          <div style="font-size:.67rem;color:rgba(255,255,255,.4)">Ensemble · ROC · Segment · MSE/MAE/R²</div>
        </div>""", unsafe_allow_html=True)
        ...
        <div style="text-align:center;padding:.8rem 0 0;font-size:.65rem;
          ...
          © 2025 <span style="color:{OR}">Amen Bank</span><br>Risque Crédit v4.0
        </div>""", unsafe_allow_html=True)
    return page
...
# ═══════════════════════════════════════════════════
def page_segment(df):
    render_header("🎯 Analyse par Segment Client")
    X_enc,y,feat_cols,cat_map,NUM,CAT=prepare_features(df)
    with st.spinner("⚙️ Calcul des scores sur tout le portefeuille…"):
        res,sc,X_tr,X_te,y_tr,y_te,_=train_models(X_enc,y,feat_cols)
    ...
    X_all=sc.transform(X_enc) if use_sc else X_enc.values
    probas_all=model.predict_proba(X_all)[:,1]

    df_seg=df.copy()
    df_seg["Score_BAD"]=probas_all*100
    df_seg["Segment"]=pd.cut(df_seg["Score_BAD"],bins=[0,25,50,75,100],
        labels=["🟢 Vert (0-25%)","🟡 Jaune (25-50%)","🟠 Orange (50-75%)","🔴 Rouge (75-100%)"])

    seg_counts=df_seg["Segment"].value_counts().sort_index()
    seg_bad=df_seg.groupby("Segment",observed=True)["Risk"].apply(
        lambda x:(x=="bad").sum()/len(x)*100)

    c1,c2=st.columns(2)
    colors_seg=[VERT_C,"#F59E0B","#F97316",ROUGE]
    with c1:
        section("Répartition par Segment")
        fig_s=go.Figure(go.Bar(x=seg_counts.index.astype(str),y=seg_counts.values,
            marker_color=colors_seg,text=seg_counts.values,textposition="outside"))
        fig_s.update_layout(paper_bgcolor="rgba(0,0,0,0)",
            plot_bgcolor="rgba(0,0,0,0)",height=350,margin=dict(t=10,b=10))
        st.plotly_chart(fig_s, use_container_width=True)
    with c2:
        section("Taux BAD Réel par Segment")
        fig_s2=go.Figure(go.Bar(x=seg_bad.index.astype(str),y=seg_bad.values,
            marker_color=colors_seg,text=[f"{v:.1f}%" for v in seg_bad.values],textposition="outside"))
        fig_s2.update_layout(yaxis_title="% BAD",paper_bgcolor="rgba(0,0,0,0)",
            plot_bgcolor="rgba(0,0,0,0)",height=350,margin=dict(t=10,b=10))
        st.plotly_chart(fig_s2, use_container_width=True)

    seg_list=sorted([str(s) for s in df_seg["Segment"].unique() if pd.notna(s)])
    cols_seg=st.columns(len(seg_list))
    for i,seg in enumerate(seg_list):
        sub=df_seg[df_seg["Segment"].astype(str)==seg]
        rb=(sub["Risk"]=="bad").sum()
        kpi(cols_seg[i],len(sub),f"{seg}\n{rb} BAD ({rb/len(sub)*100:.0f}%)",
            "📊",["kpi-success","kpi-or","","kpi-danger"][min(i,3)])

    section("Distribution des Scores par Classe Réelle")
    fig_dist=go.Figure()
    for risk,clr in [("good",VERT_C),("bad",ROUGE)]:
        sub=df_seg[df_seg["Risk"]==risk]
        fig_dist.add_trace(go.Histogram(x=sub["Score_BAD"],name=risk.upper(),
            nbinsx=40,marker_color=clr,opacity=0.7))
    fig_dist.add_vline(x=50,line_dash="dash",line_color=NOIR,annotation_text="Seuil 50%")
    fig_dist.update_layout(barmode="overlay",xaxis_title="Score BAD (%)",
        paper_bgcolor="rgba(0,0,0,0)",plot_bgcolor="rgba(0,0,0,0)",
        height=380,margin=dict(t=10,b=10),legend=dict(orientation="h"))
    st.plotly_chart(fig_dist, use_container_width=True)

    section("Analyse Financière par Segment")
    seg_fin=df_seg.groupby("Segment",observed=True).agg(
        Nb_Clients=("Risk","count"),
        Montant_Total=("Credit amount","sum"),
        Montant_Moyen=("Credit amount","mean"),
        Duree_Moy=("Duration","mean"),
        Taux_BAD=("Risk",lambda x:(x=="bad").sum()/len(x)*100)
    ).reset_index()
    seg_fin["Montant_Total"]=seg_fin["Montant_Total"].apply(lambda x:f"{x:,.0f}")
    seg_fin["Montant_Moyen"]=seg_fin["Montant_Moyen"].apply(lambda x:f"{x:,.0f}")
    seg_fin["Duree_Moy"]=seg_fin["Duree_Moy"].apply(lambda x:f"{x:.1f}")
    seg_fin["Taux_BAD"]=seg_fin["Taux_BAD"].apply(lambda x:f"{x:.1f}%")
    st.dataframe(seg_fin, use_container_width=True, hide_index=True)

    section("🎛️ Simulateur de Politique de Crédit")
    st.markdown("Ajustez le seuil et visualisez l'impact sur le portefeuille complet :")
    thresh_sim=st.slider("Seuil de décision",0.20,0.80,0.50,0.01,key="thresh_sim")
    approved=(df_seg["Score_BAD"]/100<thresh_sim).sum()
    refused =(df_seg["Score_BAD"]/100>=thresh_sim).sum()
    tb_refused=((df_seg["Score_BAD"]/100>=thresh_sim)&(df_seg["Risk"]=="bad")).sum()
    fb_refused=((df_seg["Score_BAD"]/100>=thresh_sim)&(df_seg["Risk"]=="good")).sum()
    s1,s2,s3,s4=st.columns(4)
    kpi(s1,approved,"Crédits Approuvés","✅","kpi-success")
    kpi(s2,refused, "Crédits Refusés",  "⛔","kpi-danger")
    kpi(s3,tb_refused,"BAD Détectés (bonne décision)","🎯","kpi-or")
    kpi(s4,fb_refused,"GOOD Faussement Refusés","⚠️","kpi-blue")
...
def page_data(df):
    ...
    with tab1:
        ...
        m1.metric("Lignes",len(d))
        m2.metric("Risques élevés",len(d[d["Risk"]=="bad"]))
        m3.metric("Montant moyen",f"{d['Credit amount'].mean():,.0f} TND" if len(d)>0 else "–")
        m4.metric("Durée moyenne",f"{d['Duration'].mean():.1f} mois" if len(d)>0 else "–")
        def cr(v):
        ...
        st.download_button("⬇️  Télécharger Dataset CSV",
            ...
            file_name="amen_bank_dataset.csv",mime="text/csv",key="dl_dataset")

    with tab2:
        section("Historique des Analyses de Risque")
        hist=load_historique()
        if len(hist)==0:
            st.info("📭  Aucune analyse enregistrée. Effectuez une prédiction pour voir l'historique.")
        else:
            h1,h2,h3,h4,h5=st.columns(5)
            h1.metric("Total",len(hist))
            ...
            h5.metric("Taux BAD hist.",f"{len(hist[hist['Statut']=='bad'])/len(hist)*100:.1f}%")
            st.markdown("<br>", unsafe_allow_html=True)
            fc1,fc2=st.columns(2)
            hrf =fc1.selectbox("Filtrer Décision",["Tous","BON CLIENT","RISQUE ÉLEVÉ"],key="hrf")
            hana=fc2.selectbox("Filtrer Analyste",["Tous"]+sorted(hist["Analyste"].unique().tolist()),key="hana")
            h=hist.copy()
            ...
            st.dataframe(h.style.applymap(cr_h,subset=["Statut"]).applymap(cr_d,subset=["Decision"]),
                use_container_width=True,height=420,hide_index=True)
            if len(h)>=2:
                section("Évolution du Score de Risque")
                fig_ev=go.Figure()
                fig_ev.add_trace(go.Scatter(x=list(range(1,len(h)+1)),y=h["Score_Risque_pct"].values,
                    mode="lines+markers",line=dict(color=VERT,width=2),
                    marker=dict(color=[ROUGE if s=="bad" else VERT_C for s in h["Statut"].values],
                                size=10,line=dict(color="white",width=2)),
                    fill="tozeroy",fillcolor=f"rgba(0,107,60,0.07)",name="Score"))
                fig_ev.add_hline(y=50,line_dash="dash",line_color=OR,annotation_text="Seuil 50%")
                fig_ev.update_layout(xaxis_title="N° Analyse",yaxis_title="Score (%)",
                    yaxis_range=[0,105],paper_bgcolor="rgba(0,0,0,0)",
                    plot_bgcolor="rgba(0,0,0,0)",height=320,margin=dict(t=10,b=10))
            ...
            with col_clr:
                if st.button("🗑️  Effacer l'historique",use_container_width=True,key="clr_hist"):
                    ...
                    st.success("Historique effacé."); st.rerun()
...
def main():
    ...
    elif "Segment"              in page: page_segment(df)
    elif "Données"              in page: page_data(df)
    st.markdown(f"""
    <div class="footer">
      © 2025 <span>Amen Bank Tunisie</span> — Tous droits réservés &nbsp;|&nbsp;
      Direction des Risques &nbsp;·&nbsp; Système Risque Crédit <b>v4.0 Professional</b>
      &nbsp;·&nbsp; XGBoost · LightGBM · Ensemble Voting · ROC · SHAP-ready
    </div>""", unsafe_allow_html=True)
